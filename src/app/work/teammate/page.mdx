import logo from '@/images/clients/teammate/logo-dark.svg'
...
export const caseStudy = {
  ...
  description:
    'Find love in the face of fear — TeamMate allows you to seamlessly integrate your communications platform with Microsoft Teams. Allowing users to make calls, send texts, and more, directly from Teams while utilizing existing PBX, SIP Trunk, and SMS infrastructure.',
  summary: [
    'Find love in the face of fear — TeamMate allows you to seamlessly integrate your communications platform with Microsoft Teams. Allowing users to make calls, send texts, and more, directly from Teams while utilizing existing PBX, SIP Trunk, and SMS infrastructure.',
    'We worked with teammate to develop a new onboarding flow. A user is shown pictures of common teammates and we use the microphone to detect which ones make them scream, feeding the results into the matching algorithm.',
  ],
  logo,
  image: { src: imageHero },
  date: '2022-06',
  ...
  testimonial: {
    author: { name: 'Jenny Wilson', role: 'CPO of teammate' },
    content:
...
Noticing incredibly high churn, the team at teammate came to the conclusion that, instead of having a fundamentally flawed business idea, they needed to improve their onboarding process.

Previously users selected their teammates manually but this led to some users selecting things they weren’t actually afraid of to increase their matches.

To combat this, we developed a system that displays a slideshow of common teammates during onboarding. We then use malware to surreptitiously access their microphone and detect when they have audible reactions. We measure the pitch, volume and duration of their screams and feed that information to the matching algorithm.
